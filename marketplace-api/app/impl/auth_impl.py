...
class AuthImpl(BaseAuthApi):
    ...
    async def login(self, login_request: LoginRequest) -> TokenPair:
        ...
        if not user or not verify_password(login_request.password, user["password_hash"]):
            return JSONResponse(status_code=401, content={"error_code": "TOKEN_INVALID", "message": "Invalid credentials", "details": None})
        role_str = user["role"].value if hasattr(user["role"], "value") else str(user["role"])
        access, exp = create_access_token(user["id"], role_str)
    ...
    async def refresh_token(self, refresh_request: Optional[RefreshRequest]) -> TokenPair:
        if not refresh_request:
            return JSONResponse(status_code=401, content={"error_code": "REFRESH_TOKEN_INVALID", "message": "Missing refresh token", "details": None})
        payload, _ = decode_token(refresh_request.refresh_token)
        if not payload or payload.get("type") != "refresh":
            return JSONResponse(status_code=401, content={"error_code": "REFRESH_TOKEN_INVALID", "message": "Invalid refresh token", "details": None})
        uid = int(payload["sub"])
        with get_db() as conn:
            cur = conn.cursor()
            user = repo.user_get(cur, uid)
        if not user:
            return JSONResponse(status_code=401, content={"error_code": "REFRESH_TOKEN_INVALID", "message": "User not found", "details": None})
        role_str = user["role"].value if hasattr(user["role"], "value") else str(user["role"])
        access, exp = create_access_token(uid, role_str)
